...
public class ZombieBehaviour : Obstacle, IObstacleBehaviour
{
    public void DoLogic()
    {
        ...
        {
            if (nextTile.GetPiece().GetTopFace().classData.IsCombatClass || nextTile.GetPiece().statusEffectController.IsStatusActive(PieceStatus.Stun))
            {
                AnimateObstacleHalfBack(nextStep);
                nextStep = oppositeStep;
            ...
            else
            {
                ...
                var stunTurns = 2;
                nextTile.GetPiece().statusEffectController.SetStatus(PieceStatus.Stun, stunTurns);
